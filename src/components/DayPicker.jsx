...
export function calculateDimension(el, axis, borderBox = false, withMargin = false) {
	...
	if (!borderBox) {
		size -=
			parseFloat(style[`padding${axisStart}`]) + parseFloat(style[`padding${axisEnd}`]) + parseFloat(style[`border${axisStart}Width`]) + parseFloat(style[`border${axisEnd}Width`]);
	}
...
export default class DayPicker extends Component {
	...
	onPrevMonthClick(nextFocusedDate, e) {
		...
		const translationValue = this.isVertical() ? this.getMonthHeightByIndex(0) : this.dayPickerWidth;
	...
	onNextMonthClick(nextFocusedDate, e) {
		...
		const translationValue = this.isVertical() ? -this.getMonthHeightByIndex(1) : -this.dayPickerWidth;
	...
	isDayVisible(day, newMonth) {
		...
		const lastDayOfLastMonth = month
			.clone()
	...
	renderNavigation() {
		const { navPrev, navNext, orientation, phrases } = this.props;
	...
	renderWeekHeader(index) {
		...
		for (let i = 0; i < 7; i += 1) {
			header.push(
				<li key={i} style={{ width: `${daySize}px` }}>
					<small>
						{moment()
							.weekday(i)
							.format('dd')}
		...
		return (
			<div className="DayPicker__week-header" key={`week-${index}`} style={style}>
				<ul>{header}</ul>
			</div>
		);
	}
	...
	render() {
		const { calendarMonthWidth, currentMonth, monthTransition, translationValue, scrollableMonthMultiple, focusedDate, showKeyboardShortcuts, isTouchDevice: isTouch } = this.state;
		...
		return (
			<div className={dayPickerClassNames} style={dayPickerStyle}>
				<OutsideClickHandler onOutsideClick={onOutsideClick}>
					<div className="DayPicker__week-headers" aria-hidden="true" role="presentation">
						{weekHeaders}
					</div>
