...
import { HORIZONTAL_ORIENTATION, VERTICAL_ORIENTATION, ANCHOR_LEFT, ANCHOR_RIGHT, DAY_SIZE } from '../../constants';
...
export default class SingleDatePicker extends Component {
	...
	componentDidMount() {
		this.resizeHandle = addEventListener(window, 'resize', this.responsivizePickerPosition, { passive: true });
	...
	onChange(dateString) {
		const { startDate, isOutsideRange, keepOpenOnDateSelect, onDateChange, onFocusChange, onClose } = this.props;
	...
	onDayClick(day, e) {
		...
		const { onDateChange, keepOpenOnDateSelect, onFocusChange, onClose, startDate, endDate } = this.props;
	...
	centerControl() {
		...
		if (autoScroll) {
			const el = document.getElementsByClassName('SingleDatePicker__picker')[0];
	...
	responsivizePickerPosition() {
		const { anchorDirection, horizontalMargin, withPortal, withFullScreenPortal, focused } = this.props;
	...
	renderDayPicker() {
		...
		return (
			<div // eslint-disable-line jsx-a11y/no-static-element-interactions
				...
				{withFullScreenPortal && (
					<button aria-label={phrases.closeDatePicker} className="SingleDatePicker__close" type="button" onClick={this.onClearFocus}>
	...
	render() {
		const { id, placeholder, disabled, focused, required, showClearDate, date, phrases, withPortal, withFullScreenPortal, screenReaderInputMessage } = this.props;
		...
		return (
			<div className="SingleDatePicker" onClick={this.centerControl}>
